...
## Modified version of the MicroFS for the MicroCLI
## Credits to the original author @ntoll (https://github.com/ntoll/microfs)
## The original project and this modification are licensed under the MIT License.
...
def show(filename, serial=None):
    """
    Display the contents of a file on the micro:bit.
    ...
    commands = [
        "\n".join([
            "with open('{}', 'r') as f:".format(filename),
            " content = f.read()",
            " print(content)",
        ]),
    ]
...
def rename(filename, newname, serial=None):
    """
    Renames a file on the micro:bit.
    ...
    commands = [
        "\n".join([
            ...
            "with open('{}', 'r') as f:".format(filename),
            " content = f.read()",
            "with open('{}', 'w') as f:".format(newname),
            " f.write(content)",
...
def up(filename, target=None, serial=None):
    """
...
def save(filename, target=None, serial=None):
    """
...
def exec_cmd(command=None):
    try:
        ...
        subparsers = parser.add_subparsers(
            dest="command", help="One of 'ls', 'rm', 'up' or 'save'"
        )
        ...
        if args.command == "ls":
            ...
            if args.path:
                ...
                print('rm: missing filename. (e.g. "./microCLI -rm foo.txt")')
                sys.exit(84)
        elif args.command == "up":
            if args.path:
                up(args.path, args.target)
            else:
                print('up: missing filename. (e.g. "./microCLI -up foo.txt")')
                sys.exit(84)
        elif args.command == "save":
            if args.path:
                save(args.path, args.target)
            else:
                print('save: missing filename. (e.g. "./microCLI -save foo.txt")')
                sys.exit(84)
        elif args.command == "show":
            if args.path:
                show(args.path)
            else:
                print('show: missing filename. (e.g. "./microCLI -show foo.txt")')
                sys.exit(84)
        elif args.command == "rename":
            if args.path:
                rename(args.path, args.target)
            else:
                print('rename: missing filename. (e.g. "./microCLI -rename foo.txt bar.txt")')
                sys.exit(84)
